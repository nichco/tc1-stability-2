...
"""
# size and values are parameter
size = 4
val = np.random.rand(size, size)

# Run model
sim = csdl_om.Simulator(Eig(size=size, val=val))
sim.prob.run_model()
...
print('A: \n', sim.prob['A'])
print()
w_check, v_check = np.linalg.eig(sim.prob['A'])
print('eigenvalues real (CSDL)  :', sim.prob['e_real'])
print('eigenvalues real (check) :', np.real(w_check))
print()
print('eigenvalues imag (CSDL)  :', sim.prob['e_imag'])
print('eigenvalues imag (check) :', np.imag(w_check))
print()
